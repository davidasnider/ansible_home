...
The `GITHUB_TOKEN` environment variable is required for certain infrastructure automation scripts (e.g., in `infrastructure/__main__.py`). Ensure it is loaded into your environment. Tests verify that an error is correctly raised if it is missing, using both fully isolated subprocess environments and in-process testing (via `monkeypatch` and `sys.modules` manipulation) to capture accurate code coverage for load-time logic.
