...
- Subprocess isolation and in-process module reload testing (e.g., verifying `GITHUB_TOKEN` requirement and ensuring coverage)
...
6. **Environment Isolation**: Use idiomatic `pytest` fixtures like `tmp_path` and `monkeypatch.delenv` for strict environment isolation in subprocess execution. For testing load-time logic that requires coverage tracking, use in-process testing by combining `monkeypatch` and `sys.modules` manipulation to force a clean module reload.
