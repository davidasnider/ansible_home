...
## 2024-09-14 - Optimize Ansible Playbook Architecture Gathering
**Learning:** Ansible task execution carries significant setup, connection, and parsing overhead. Using `ansible.builtin.command: dpkg --print-architecture` inside a playbook introduces an unnecessary shell execution step for data that is already collected automatically in `ansible_facts`.
**Action:** Replace shell calls for system architecture with mapped native facts (`{{ {'x86_64': 'amd64', 'aarch64': 'arm64'}.get(ansible_facts['architecture'], ansible_facts['architecture']) }}`) to eliminate task execution overhead and speed up playbook runs.
