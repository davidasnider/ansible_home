...
## 2026-09-15 - Replace dpkg --print-architecture with ansible_facts
**Learning:** Running `dpkg --print-architecture` via a shell command creates unnecessary execution overhead in Ansible.
**Action:** Use native jinja mapping on `ansible_facts['architecture']` (e.g., `{'x86_64': 'amd64', ...}[ansible_facts['architecture']]`) to map the architecture without spawning a shell process.
