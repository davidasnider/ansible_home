...
## 2024-09-05 - Consolidate commands into single shell task
**Learning:** To optimize Ansible loops over command/shell tasks, consolidate the iterations into a single multiline `ansible.builtin.shell` block to bypass per-iteration task parsing and setup overhead.
**Action:** Replace `ansible.builtin.command` loops with a multiline `ansible.builtin.shell` script whenever running sequential simple commands.
