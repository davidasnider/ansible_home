...
## 2026-09-08 - Command Injection in Eval Alias
**Vulnerability:** The `opload` alias used `eval "$(cat ~/.env | op inject)"` to load environment variables. This creates a severe command injection vulnerability if a secret stored in 1Password contains unescaped characters or quotes (e.g., `value"; echo pwned; echo "`). The `eval` or `source` commands execute the substituted text as arbitrary shell code.
**Learning:** Using `eval` or `source` on output from a secret manager (or any untrusted source) blindly executes whatever is generated, allowing for command injection via malicious or malformed secret payloads.
**Prevention:** Avoid `eval` and `source` for processing untrusted environment variable assignments. Instead, use a secure parsing loop (e.g., `while IFS= read -r line; do ... export "$key"="$val"; done`) to read keys and values line-by-line and safely construct `export` statements using bash parameter expansion and explicit string assignment.
