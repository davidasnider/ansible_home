...
## 2024-09-19 - Command Injection Risk via eval on Secret Outputs
**Vulnerability:** The alias `opload` loaded environment variables by executing `eval "$(cat ~/.env | op inject)"`. Using `eval` on the output of secret managers allows any unescaped characters within the secret value to be interpreted as bash commands, leading to critical command injection vulnerabilities.
**Learning:** Evaluating (`eval`) dynamic output derived from secrets allows attackers to execute arbitrary shell commands if they can control the content of the secrets or the templates processing them.
**Prevention:** Never use `eval` or `source` directly on outputs from secret managers or templates. Instead, securely parse the stream by iterating over it with a `while IFS= read -r line` loop, perform safe string manipulation to remove any prefix (like `export `) or surrounding quotes, and use explicit assignment (e.g., `export "$key"="$val"`). This limits variable injection vectors while preventing unintended execution.
