...
## 2024-05-24 - [CRITICAL] Prevent Command Injection from Secret Managers
**Vulnerability:** Using `eval "$(cat ~/.env | op inject)"` causes severe command injection vulnerabilities. If any loaded secret contained unescaped shell metacharacters or spaces, `eval` would execute them as arbitrary commands.
**Learning:** Never use `eval` or `source` to load dynamically generated environment variables or secrets from templated outputs. Even if the secrets originate from a trusted secret manager (like 1Password), their contents are unpredictable and can break out of expected string boundaries when evaluated.
**Prevention:** Use a secure string parsing loop (e.g., `while IFS= read -r line || [[ -n "$line" ]]`) combined with parameter expansion to safely extract and export keys and values without invoking the shell evaluator.
