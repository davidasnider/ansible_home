...
## 2026-09-10 - Command Injection in Eval Secret Loading
**Vulnerability:** The alias `opload` used `eval "$(cat ~/.env | op inject)"`, making it vulnerable to command injection if secret values contain unescaped shell metacharacters.
**Learning:** Shell `eval` executes string content directly. When secrets contain characters like backticks, semicolons, or quotes, they can break out of variable assignment context and execute arbitrary code.
**Prevention:** Avoid `eval` for loading secrets. Instead, read the secrets line-by-line using `while IFS= read -r line`, strip formatting, and use `export` with properly quoted variable assignments.
