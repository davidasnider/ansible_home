...
**Prevention:** Create the file with restrictive permissions from creation time using a subshell and umask (e.g., `(umask 077; echo "secret" > "file")`). This closes the post-create chmod window by ensuring the file is never written with permissive permissions, though it is not an atomic write/update.
