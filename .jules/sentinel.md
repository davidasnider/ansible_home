...
## 2024-10-18 - Command Injection via 1Password Shell Env Export
**Vulnerability:** In both `roles/workstation/tasks/zshrc-mac` and `roles/workstation/tasks/zshrc-linux`, the `opload` alias used `eval` on the unfiltered output of `cat ~/.env | op inject`. A maliciously crafted `.env` variable value like `BAD_SECRET="value; echo 'command injection'"` would execute arbitrary code since `eval` would interpret it as bash commands.
**Learning:** Loading and evaluating shell environment exports generated from external files, especially those processed by tools like `op inject` and templating engines, exposes the shell to command injection if unescaped characters or payload commands exist in the value strings.
**Prevention:** Avoid `eval` for evaluating shell exports outputted from templated or dynamically injected text files containing secrets. Instead, read the secrets safely via string manipulation (e.g. `while IFS= read -r line`) and export them natively within the shell function using `export "$key"="$value"`.
