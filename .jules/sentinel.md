...
## 2025-01-20 - Prevent Command Injection in 1Password opload
**Vulnerability:** The `opload` alias used `eval "$(cat ~/.env | op inject)"`, which allowed command injection if an environment variable contained unescaped bash control characters.
**Learning:** Using `eval` to load environment variables from secret managers exposes the environment to arbitrary code execution if values are maliciously crafted.
**Prevention:** Use a secure string parsing loop (`while IFS= read -r line`) to extract and export keys and values safely, avoiding `eval` for dynamic content.
